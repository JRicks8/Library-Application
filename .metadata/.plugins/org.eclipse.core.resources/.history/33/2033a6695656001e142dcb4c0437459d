...
public abstract class BookList{
	...
	//returns a new arraylist that is sorted by book_id. 
	protected ArrayList<Book> NewIDSortedList(ArrayList<Book> _bookList){
		...
		for(int a=0;a<newList.size();a++) {
			...
			for(int b=a+1;b<newList.size();b++) {
				float bookAID = Float.parseFloat(newList.get(a).book_id);
				float bookBID = Float.parseFloat(newList.get(b).book_id);
	...
	protected abstract void SortByAuthor(boolean _ascending);
	
	//sorts books by publication year, with earlier books coming first.
	protected abstract void SortByPublicationYear(boolean _ascending);
