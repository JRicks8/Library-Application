...
public class BookArrayList extends BookList{
	...
	public void SortByAuthor(boolean _ascending) {
		...
		for(int a=0;a<books.size();a++) {
			...
			for(int b=a+1;b<books.size();b++) {
				...
				for(int c=0;c<bookATitle.length() && c<bookBTitle.length();c++) {
					...
					character for both strings comes first if ascending, and last if descending.*/
					
					//if bookA[c] has a lower value than bookB[c], book A comes first if ascending and the comparison can stop.
	...
	//sorts books by publication year, with earlier books coming first if it is ascending.
	public void SortByPublicationYear(boolean _ascending) {
		...
		for(int a=0;a<books.size();a++) {
			...
			for(int b=a+1;b<books.size();b++) {
				...
				//if book b has an earlier(ascending)/later(descending) year than book a, swap their positions.
				if((bookAYear > bookBYear && _ascending) || (bookAYear < bookBYear && _ascending == false)) {
					SwapPositions(a,b);
				}
			}
		}
