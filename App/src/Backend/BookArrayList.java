...
public class BookArrayList extends BookList{
	...
	public void SortByPublicationYear() {
		...
		//if an earlier suitable publication year is found for the position, swap the position of that book and the book currently at the index.
		...
		for(int a=0;a<books.size();a++) {
			//at each index, compare every book that is at a higher index.
			for(int b=a+1;b<books.size();b++) {
				float bookAYear = books.get(a).original_publication_year;
				float bookBYear = books.get(b).original_publication_year;
